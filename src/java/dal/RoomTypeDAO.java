...
public class RoomTypeDAO {
    ...
    // ✅ COUNT active room types (dùng cho nút VIEW ALL)
    ...
    public List<RoomType> getActiveRoomTypesForHome(int limit, String priceSort) {

    List<RoomType> list = getActiveRoomTypesForHome(limit);

    if (list == null || list.isEmpty()) return list;
    if (priceSort == null || priceSort.isBlank()) return list;

    boolean allNull = true;
    for (RoomType rt : list) {
        if (rt.getPriceToday() != null) {
            allNull = false;
            break;
        }
    }
    ...
    list.sort((a, b) -> {
        var pa = a.getPriceToday();
        var pb = b.getPriceToday();
        ...
        return "desc".equalsIgnoreCase(priceSort) ? -cmp : cmp;
    });
