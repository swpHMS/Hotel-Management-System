        ...
        try {
            ...
            if (existingUser != null) {
                if (existingUser.getStatus() == 1) {
                    ...
                    try {
                        ...
                        e.printStackTrace();
                        request.setAttribute("error", "Failed to resend the activation email. Please try again later.");
                    }
                }
            ...
            if (success) {
                try {
                    ...
                    e.printStackTrace();
                    request.setAttribute("message", "Account created successfully, but failed to send verification email. Please try resending the verification link.");
                    keepFormData(request, fullName, email, phone);
                    request.getRequestDispatcher("/view/auth/register-customer.jsp").forward(request, response);
                }
            } else {
