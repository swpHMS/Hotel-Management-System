...
public class VNPayReturnServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean isOk = validHash
                && "00".equals(responseCode)
                && "00".equals(transactionStatus);

        // 6) Lấy holdId từ vnp_TxnRef: holdId_timestamp
        ...
        long amountVnp = 0;
        try {
            amountVnp = Long.parseLong(request.getParameter("vnp_Amount"));
        } catch (Exception ignored) {
        }
        ...
        // 8) Load dữ liệu từ session để finalize
        ...
        if (isBlank(customerEmail)) {
            ...
            if (ue != null && !isBlank(String.valueOf(ue))) {
                customerEmail = String.valueOf(ue).trim();
            }
        }
        ...
        if (isOk && holdId > 0) {
            try {
                ...
                BigDecimal total = (!isBlank(totalS))
                        ...
                        : paid.multiply(new BigDecimal("2"));

                Integer userId = null;
                String fullName = null;

                Object ua = session.getAttribute("userAccount");
                if (ua != null) {
        ...
        // Nếu thanh toán fail hoặc finalize fail thì mới forward JSP
        String bookingCode = "#N/A";
        if (holdId > 0) {
            bookingCode = "#HOLD-" + holdId;
        }
