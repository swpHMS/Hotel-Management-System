...
<head>
  ...
  <style>
    ...
    .card{ background:#fff; border:1px solid rgba(15,23,42,.10); border-radius:16px; padding:28px;
      box-shadow:0 8px 24px rgba(15,23,42,.06); margin-bottom:18px; }
    .grid-2{ display:grid; grid-template-columns:1fr 1fr; gap:16px; }
    .grid-3{ display:grid; grid-template-columns:1fr 1fr 1fr; gap:16px; }
    @media (max-width: 900px){ .grid-2,.grid-3{ grid-template-columns:1fr; } }

    .label{ font-size:11px; letter-spacing:.14em; text-transform:uppercase; color:rgba(15,23,42,.45); font-weight:800; margin-bottom:6px; }
    .control{ width:100%; border:1px solid rgba(15,23,42,.12); border-radius:12px; padding:10px 12px; outline:none; background:#fff; }
    .control:focus{ box-shadow:0 0 0 3px rgba(37,99,235,.15); border-color:rgba(37,99,235,.45); }
    .readonly{ background:#f8fafc; border-color:rgba(15,23,42,.08); color:rgba(15,23,42,.45); cursor:not-allowed; }
    .hint{ font-size:11px; color:rgba(15,23,42,.45); font-style:italic; margin-top:6px; }

    .actions{ display:flex; justify-content:flex-end; gap:12px; padding-top:16px; border-top:1px solid rgba(15,23,42,.08); }
    .btn{ padding:10px 18px; border-radius:12px; font-weight:800; border:1px solid rgba(15,23,42,.18); background:#fff; cursor:pointer; }
    .btn-primary{ background:#2563eb; color:#fff; border-color:#2563eb; box-shadow:0 10px 20px rgba(37,99,235,.18); }

    .field-error{ color:#dc2626; font-size:12px; font-weight:700; margin-top:6px; }
  ...
  <%@ include file="/view/admin_layout/sidebar.jsp" %>
    ...
    <div class="page-wrap">
      ...
      <div style="margin-bottom:14px;">
        <div style="color:rgba(15,23,42,.55); font-size:13px;">
          Admin &gt; User Management &gt; <b>Edit Profile</b>
        </div>
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; gap:12px; margin-top:6px;">
          ...
          <div style="display:flex; gap:10px;">
            ...
            <a class="btn btn-primary" href="${pageContext.request.contextPath}/admin/access-control?userId=${staff.userId}">Access Control</a>
          </div>
        </div>
      </div>
